...
#define GETOPT_FIELD(r, type, elem) type elem;

// Stringify flag/option characters for getopt spec string: "a" / "d:"
#define GETOPT_SPEC(r, suffix, elem) BOOST_PP_STRINGIZE(elem) suffix

// Generate switch case branches: case 'a': opts.a = val; break;
#define GETOPT_CASE(r, val, elem)                                                                  \
  case BOOST_PP_STRINGIZE(elem)[0]:                                                                \
    opts.elem = val;                                                                               \
    break;

#define GETOPT_EACH(macro, data, seq) BOOST_PP_SEQ_FOR_EACH(macro, data, seq)
...
#define GETOPT(list, flags, options)                                                               \
  ...
  do {                                                                                             \
    ...
    const char *const spec = GETOPT_EACH(GETOPT_SPEC, "", BOOST_PP_VARIADIC_TO_SEQ flags)          \
      GETOPT_EACH(GETOPT_SPEC, ":", BOOST_PP_VARIADIC_TO_SEQ options) "h";                         \
