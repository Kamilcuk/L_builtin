...
/// Traverse a sentinel-terminated `l_flag_entry_t` C table in reverse,
/// collecting the names of all flags whose bit is set in `flags`.
...
const FCNTL_CMD: CmdDesc = CmdDesc::new(
    c"fcntl",
    c"getfl [-v VAR] FD | setfl FD FLAGS | getfd [-v VAR] FD | setfd FD FLAGS | dup [-v VAR] [-c] FD [START]",
    c"\
...
const FCNTL_TABLE: crate::intlookup::U64::IntLookup<SubcommandFn, 5> =
