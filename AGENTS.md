...
- `make readme` → regenerates `doc/commands.md` (the full per-subcommand
  reference) from `L_builtin <subcommand> -h`. Also strips any leftover
  `<!-- *_GEN_* -->` blocks from `README.md` and rewrites its
  `## Table of Contents` to link to `doc/commands.md`. Requires a built `.so`
  plus `uv` (the `markdown-it-py` library is fetched automatically). Edit
  `scripts/gen_readme.py` to change generation. Requires a built `.so`.
